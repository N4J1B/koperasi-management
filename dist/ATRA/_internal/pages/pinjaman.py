...
class PinjamanPage(BasePage):
    ...
    def _build(self):
        ...
        # Preview angsuran
        prev = tk.Frame(form_card, bg="#EBF8FF", padx=10, pady=6)
        prev.grid(row=8, column=0, columnspan=2, sticky="ew", padx=12, pady=(4, 0))
        tk.Label(prev, text="Estimasi Angsuran/bln:", font=("Arial", 8),
                 bg="#EBF8FF", fg=C_GRAY).pack(anchor="w")
        self._v_preview = tk.StringVar(value="-")
        tk.Label(prev, textvariable=self._v_preview, font=("Arial", 11, "bold"),
                 bg="#EBF8FF", fg=C_BLUE).pack(anchor="w")
